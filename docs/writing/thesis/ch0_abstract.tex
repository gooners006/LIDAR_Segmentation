...
partial scores best, ranking a network that does nothing above one that completes.

The central contribution of this thesis is a valid metric: a donor-frame coverage score
for how much of the surface seen only in other frames of the same static car a
...
real LiDAR cars a PCN network recovers genuinely unseen surface well above the
mirror and tightens amodal boxes on static, gate-passed cars --- a benefit that
partially replicates on a pre-registered held-out sequence. Along the way, we find the
detection front-end recall-limited by clustering rather than the classifier, and
synthetic pretraining redundant once real training clusters are plentiful. The thesis
thus makes a quantity that could not be measured on real LiDAR measurable, giving a way
to test whether completion helps rather than assume it.
