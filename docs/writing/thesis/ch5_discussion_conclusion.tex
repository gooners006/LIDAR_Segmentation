...
Chapter~\ref{ch:introduction} posed three research questions. Each is answered by a specific chapter, and each
...
(Findings~\#32/\#37). This is a validated way to score completion quality on
...
The first half is answered in Section~\ref{sec:recall-analysis}, the second in Chapter~\ref{ch:evaluation}. Recall is limited not by
...
thesis makes three claimed contributions --- all methodological or diagnostic rather than
architectural, and none a claim of novelty for the pipeline itself.
...
\begin{enumerate}
  ...
  pre-registered held-out sequence (Findings~\#26/\#29/\#32/\#37/\#42; Section~\ref{sec:donor-validation}).
  This is the completion half's central contribution.
  ...
  findings in their own right (Findings~\#21/\#23/\#24/\#34; Section~\ref{sec:recall-analysis}).
  \item \textbf{Evidence that synthetic pretraining is redundant for the classifier at
  scale,} which directly replaces a contribution promised in the proposal
  (Findings~\#25/\#30; Chapter~\ref{ch:evaluation}).
\end{enumerate}
...
exact-output and voxel-resolution tiers, because the post-ground object cloud is
intrinsically sparse. It is worth separating an implementation floor from an algorithmic
...
the measurement to say so: the clustering benchmark (Finding~\#43) showed that no
...
contributes a validated real-data metric and pre-registered, partially
