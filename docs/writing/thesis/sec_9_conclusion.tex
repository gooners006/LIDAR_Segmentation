...
% use typed numerals ("Chapter~\ref{ch:completion-eval}", "Section~\ref{sec:results-heldout}", "Table~4.5") because the chapter files
...
real-data metric existed at the outset. Chapter~\ref{ch:discussion} drew the boundary around each result.
...
The measurement question is answered in Chapter~\ref{ch:completion-eval}. Reference-based Chamfer distance against
...
Also answered in Chapter~\ref{ch:completion-eval}, and reported exactly as it fell. On real sequence-08 cars the
...
The first half is answered in Chapter~\ref{ch:recall-bottleneck}, the second in Chapter~\ref{ch:detection-eval}. Recall is limited not by
...
\begin{enumerate}
  ...
  pre-registered held-out sequence (Findings~\#26/\#29/\#32/\#37/\#42; Chapter~\ref{ch:completion-eval}).
  ...
  findings in their own right (Findings~\#21/\#23/\#24/\#34; Chapter~\ref{ch:recall-bottleneck}).
  ...
  (Findings~\#25/\#30; Chapter~\ref{ch:detection-eval}).
\end{enumerate}
...
Chapter~\ref{ch:discussion} named the boundaries of the present results. Three of them are the natural next
