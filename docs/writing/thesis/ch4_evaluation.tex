...
(\texttt{src/evaluate.py}): ground removal, HDBSCAN clustering, the geometric
...
points is an optimum --- while the completion scores \emph{worst}, penalised for every
...
\begin{table}[H]
  ...
  \caption[Reference-free metrics vs.\ the donor metric]{The standard
    ...
    unseen surface. Computed by \texttt{scratchpad/ucd\_uhd\_compare.py}.}
...
\emph{under}-extends length. The completion's length geometry
...
centroid and radius --- isolates its effect on the \emph{same} checkpoint
...
\paragraph{Length geometry.} Completion \emph{under}-extends the occluded far end
