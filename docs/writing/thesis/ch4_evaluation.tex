...
(Section~\ref{sec:results-detection}), and the completion results that carry the
thesis's contribution --- the donor-frame coverage metric and what it reveals
about real-data completion (Section~\ref{sec:results-completion}). Two further
...
\begin{itemize}
  \item \textbf{Sequence 08 (reported operating point; \num{4071} frames).} The
    ...
    of the final classifier. Sequence~08 was never used to tune the
  ...
  \item \textbf{Sequence 00 (completion held-out replication).} Sequence~00 is
    ...
    model is what matters; the test is a paired comparison over its
    \num{45} well-observed static cars (Section~\ref{sec:results-heldout}), not a
    frame-count sweep. Its classical pipeline parameters
...
recall of approximately \num{0.54} against all annotated cars
...
baselines on the same inputs: the \emph{raw partial} (the floor --- it adds no
unseen surface by construction), its \emph{symmetry mirror} (a cheap,
hallucination-free way to add surface that a learned model must beat), and, for
architecture, \emph{PoinTr}; the standard reference-free metrics UCD and UHD
(Section~\ref{sec:bg-completion}) are reported alongside the donor metric in
...
\begin{table}[H]
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    Reported (final)        & 0.905 & 0.730 & 0.808 & 0.912 & \num{25478} / \num{2676} / \num{9444} \\
    Held-out (LOSO fold-08) & 0.881 & 0.737 & 0.803 & 0.912 & \num{25733} / \num{3477} / \num{9189} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Validation of the Donor Metric}
...
\begin{table}[H]
  \centering
  \caption[Four donor-metric validation checks]{The four validation checks for the donor metric, and the observed
    outcome (seq~08, per-car medians, $n=39$; the metric's original acceptance
    run, on the production path as it stood before the
  ...
  \begin{tabular}{p{0.44\linewidth} p{0.48\linewidth}}
    ...
    Check & Outcome \\
    \midrule
    (a) The raw partial must rank last on coverage (it covers no novel
...
\subsubsection{The Per-Band Hallucination Guard}
...
(Table~\ref{tab:d2}): on the compact band the length prior over-extended
...
bar there is effectively ``do not leave the box at all,'' which no
...
(Section~\ref{sec:results-heldout}). Throughout, the unit of analysis is the car: because the
...
\subsection{Real-Data Surface Coverage}
...
on the production path before the Section~\ref{ch:completion-method} length prior; the length
...
\begin{figure}[htbp]
  ...
  \caption[Donor metric on six seq-08 cars]{The donor metric in bird's-eye view on six seq-08 cars, spanning the
...
\subsubsection{Comparison with Reference-Free Completion Metrics}
\label{sec:reffree-compare}

The donor metric earns its place only if the field's existing reference-free
measures cannot already do the job. The established ones --- the unidirectional
Chamfer and Hausdorff distances (UCD, UHD; Section~\ref{sec:bg-completion}) --- score
a completion by how closely it reproduces its own partial \emph{input}. Computed on
the same \num{39} cars (per-car medians, input\,$\rightarrow$\,cloud direction), they
invert the ranking the task calls for (Table~\ref{tab:reffree-compare}): the raw
partial and its mirror score a perfect \num{0} --- any cloud that contains the input
points is an optimum --- while the completion scores worst, penalised for every
point it adds. The gap is not marginal: the raw partial's median forward UCD is
\SI{0}{\meter} against the completion's \SI{0.090}{\meter} ($p<10^{-11}$, Wilcoxon
across cars). Yet the completion is the only cloud that recovers unseen surface, as the
donor coverage column (repeated from Table~\ref{tab:donor-coverage}) shows. UCD and UHD
measure input fidelity, so their optimum is the completion that does nothing; this is
why the unpaired-completion literature pairs them with a separate plausibility term, and
why a metric restricted to the never-observed surface is needed here.

\begin{table}[H]
  \centering
  \caption[Reference-free metrics vs.\ the donor metric]{The standard
    reference-free metrics against the donor metric (seq~08, \num{39} cars, per-car
    medians). UCD/UHD are computed input\,$\rightarrow$\,cloud (lower is ``better'':
    closer reproduction of the input), so the raw partial and mirror --- which contain
    the input --- are their trivial optima and the completion is scored worst. Donor
    \texttt{cov@0.1} (higher is better) is the only column that credits recovered
    unseen surface.}
  \label{tab:reffree-compare}
  \begin{tabular}{lcccc}
    \toprule
    Cloud & UCD (m) $\downarrow$ & UHD$_{\max}$ (m) $\downarrow$ & UHD$_{p95}$ (m) $\downarrow$ & donor \texttt{cov@0.1} $\uparrow$ \\
    \midrule
    raw partial        & 0.000          & 0.000          & 0.000          & 0.000 \\
    mirrored           & 0.000          & 0.000          & 0.000          & 0.043 \\
    \textbf{completed} & 0.090          & 0.286          & 0.183          & \textbf{0.304} \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Downstream Utility: The Amodal Bounding Box}
\label{sec:results-box}

Surface coverage shows the completion is geometrically faithful; the box
evaluation shows it is useful. For each true-positive (frame, car) pair on
the well-observed static cars we fit an oriented box to the raw partial and to the
completion with the same fitter used to build the amodal ground-truth
...
\paragraph{Length: a regression the geometry corrected.} In
Table~\ref{tab:box-quality} length is neutral, which was too generous a reading of an
untreated defect: a partial truncated at the occluded far end completes short, so it
under-extends length. The completion's length geometry
(Section~\ref{ch:completion-method}) corrects this --- on the promoted configuration the
final completed box reaches median BEV IoU \num{0.771} against the raw partial's
\num{0.725}, with $|\Delta L|$ improved to \num{0.304} and centre error to \num{0.184}.
The ablation of the length prior that establishes this is reported with the completion
ablations (Section~\ref{sec:abl-completion}).
...
\subsection{Moving Cars: A Plausibility Check}
...
the \raisebox{0pt}[0pt][0pt]{$\sim$}\SI{54}{\percent} rate understates
...
\begin{figure}[htbp]
  ...
  \caption[Completions of moving-car tracks on seq~08]{Bird's-eye completions of moving-car tracks on seq~08 (blue partial
    ...
    axis-aligned plausibility measure rather than of motion.}
  \label{fig:movers}
\end{figure}

\subsection{A Pre-Registered Held-Out Replication}
...
method generalises therefore has to run on a sequence that took no part in
...
static cars to test); the two primary metrics were fixed as BEV box IoU
...
the completion claim, which is a paired raw-vs-completed comparison on
...
--- but seq~00's well-observed set contains no long cars
...
\paragraph{Stage decomposition: classifier and track-filter contributions.}
...
\paragraph{Clustering method: cost and benefit of HDBSCAN.}
...
These ablations isolate the effect of each completion-pipeline choice on the final
output; except where noted they vary one component of the completion path
(Section~\ref{ch:completion-method}) and re-score the same detections, with the
completion network itself unchanged.
...
(Section~\ref{sec:completion-normalisation}). Ablating that step against a naive
alternative --- aligning the partial with a 3D PCA frame and normalising by its own
centroid and radius --- isolates its effect on the same checkpoint
(Figure~\ref{fig:deblob}): the canonical-frame normalisation lifts the fitted box's BEV
IoU at every input density (\num{0.32}$\to$\num{0.73} sparse, \num{0.25}$\to$\num{0.74}
mid, \num{0.47}$\to$\num{0.85} dense), while the naive frame leaves the box mis-oriented
and mis-scaled off the ground-truth box.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{fig62_deblob/box_overlay_deblob_08.png}
  \caption[Canonical-frame vs.\ naive normalisation]{Canonical-frame versus naive normalisation in bird's-eye view at
    ...
    amodal ground-truth box (black) over the raw points (grey). A naive PCA +
    partial-radius normalisation (red) is mis-oriented and mis-scaled --- the 3D PCA picks an
    arbitrary principal axis rather than gravity --- while the
    canonical-frame box (green) aligns with the GT, lifting BEV box IoU at every
    density: \num{0.32}$\to$\num{0.73} (sparse), \num{0.25}$\to$\num{0.74} (mid),
    \num{0.47}$\to$\num{0.85} (dense) --- per-panel values, not an aggregate
    finding. The figure measures pose and scale
...
completion as a clean single-object cluster. An eleven-sequence ablation switches it off
and then on and scores each output by the plausibility box (the footprint
length/width/height check): the fraction of plausible completions rises from \num{0.473}
to \num{0.686} pooled, a \num{+0.213} shift positive on every one of the eleven sequences
(per-sequence \num{+0.081} to \num{+0.319}). The gate carries a measured cost, reported
rather than hidden: it drops \SI{15.5}{\percent} of otherwise-plausible cars (\num{356}
pooled) as false rejections. On the same clean-gated tracks the completion precision rises
from \num{18}/\num{47} (\SI{38}{\percent}) to \num{18}/\num{26} (\SI{69}{\percent}).

\paragraph{Length geometry.} Completion under-extends the occluded far end
--- the one box dimension the raw-versus-completed test of
Section~\ref{sec:results-box} rated neutral. Table~\ref{tab:length-geometry} ablates
the longitudinal centre push on the promoted configuration ($n=40$): the completion
with no length correction, the fixed \SI{4.14}{\meter} population prior, and the final
per-car estimate, each against the raw partial.
...
the \emph{signed} length shows the completion systematically under-extends
...
completion without a length prior makes box length worse than the
...
(\nth{90} percentile $+\SI{0.45}{\meter}$) improved both coverage and the
...
\paragraph{A bounded negative on the residual shortfall.}\label{sec:completion-step1c} Targeting each car's
true length still leaves completions short on normal and long cars, which
...
rescale). It was investigated under pre-registration (the
...
because it bounds the result: the residual far-end shortfall on long cars
...
small (Chamfer \SI{0.161}{}$\to$\SI{0.153}{\meter}), and on the same clean-gated real
tracks the two are statistically indistinguishable (\num{18}/\num{26} versus
\num{16}/\num{26} plausible completions, the difference within threshold noise), because
the residual centroid-estimation gap is architecture-independent. PCN is therefore kept
off the shelf; the completion result is set by input quality and geometry, not by the
choice of network.
...
($R=\num{0.730}$ on sequence~08). This section explains why the recall is
...
already reaches recall \num{0.775}, which is higher than the full pipeline's
...
noise), or \emph{all-noise}. A separate, cross-cutting flag marks merged cars
...
\begin{table}[H]
  ...
  \caption[Ground-truth cars across HDBSCAN clusters]{Distribution of ground-truth cars across HDBSCAN clusters
    ...
    to the denominator; split is the dominant failure mode and merging
...
below the recall the pipeline achieves on the same configuration
...
\SI{37.1}{\percent} of cars split, the great majority of split cars are not
...
(\SI{37.7}{\percent}~$\rightarrow$~\SI{29.9}{\percent}) with zero new
