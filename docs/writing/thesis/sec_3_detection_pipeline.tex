...
% Custom siunitx units for per-frame timing (\frame is otherwise the LaTeX
% kernel box-frame command, not a unit).
\DeclareSIUnit{\frame}{frame}
\DeclareSIUnit{\fps}{fps}

\begin{document}

\section{The Detection Pipeline}
\label{ch:pipeline}

This chapter describes the car-detection pipeline that produces every detection
evaluated in Chapter~4 and every cluster completed in Chapters~6 and~7. The
...
(Section~\ref{sec:pipeline-classifier}) --- and it is what lets Chapter~5
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[ms, right=2mm of pre]     {\SI{127}{\milli\second} (14\%)};
    \node[ms, right=2mm of ground]  {\SI{163}{\milli\second} (18\%)};
    \node[ms, right=2mm of cluster] {\SI{502}{\milli\second} (54\%)};
    \node[ms, right=2mm of geo]     {\SI{46}{\milli\second} (5\%)};
    \node[ms, below=0.5mm of comp.south, text width=34mm, align=center]
      {\footnotesize\itshape +\SI{19}{\milli\second} per completed car};
    \node[ms, left=2mm of clf]      {\SI{74}{\milli\second} (8\%)};
  ...
  \caption{The detection pipeline. Learned stages are shaded; all others are
    ...
    \texttt{output/experiments/timing/timing\_seq08\_full\_n4068.json}), and are
    measured, not estimated. The total is
    \SI{921}{\milli\second\per\frame} ($\approx$\SI{1.1}{\fps}); the two learned
    stages together account for only \SI{93}{\milli\second} (\SI{10}{\percent}) ---
    the classical clustering and ground-removal stages dominate. Completion runs
    off the tracked detections and is optional; its cost is per completed car,
    not per frame.}
  \label{fig:pipeline}
\end{figure}
...
\SI{921}{\milli\second\per\frame} the pipeline is roughly \SI{1.1}{\fps}, an order
of magnitude below sensor rate; we characterise this cost honestly in the runtime
section of Chapter~4 rather than claiming real-time capability. The one
observation worth making here is where the cost lives: clustering alone is
\SI{54}{\percent} of the frame budget, and the two learned components combined
are only \SI{10}{\percent}. The expensive parts of this pipeline are classical,
not neural.
...
The order of the last two steps matters for cost. In the frozen configuration
...
points. This ordering was adopted as part of a runtime optimisation that also
improved detection (Finding~\#34, discussed in Chapter~4); it is behaviour-changing,
so we flag it here rather than presenting the two orderings as equivalent.
Preprocessing costs \SI{127}{\milli\second\per\frame} in total, split across the
$z$-filter (\SI{55}{\milli\second}), denoising (\SI{55}{\milli\second}), and
downsampling (\SI{17}{\milli\second}).
...
removal costs \SI{163}{\milli\second\per\frame} (\SI{18}{\percent} of the frame).
...
and Chapter~4 reports a controlled benchmark (Finding~\#43) confirming it beats
...
being perfectly level. The filter costs \SI{46}{\milli\second\per\frame}
(\SI{5}{\percent}). Its individual gates are ablated in Chapter~4; here we note
...
\paragraph{Architecture.} The network has two branches and about \num{80000}
...
the two-stage machinery in Chapter~4 as an \emph{ablation}, because a controlled
...
contribution. The classifier costs \SI{74}{\milli\second\per\frame} (about
\SI{1.7}{\milli\second} per cluster over the \num{43} clusters in a frame).
...
in Chapter~4. The tracker itself is effectively free at
...
(Figure~\ref{fig:pipeline}, right) is the subject of Chapter~6; the evaluation of
the detections themselves is Chapter~4.
