...
% ("Section~7.2", "Chapter~6") rather than \ref{}; only within-file refs use \ref.
%
...
\label{ch:completion-eval-results}

Section~7.2 established a metric that measures, on real cars, how much
...
the paired design of Section~7.2.
...
on the production path \emph{before} the Chapter~6 length prior; the length
...
\begin{figure}[htbp]
  ...
  \caption{The donor metric in bird's-eye view on six seq-08 cars, ordered
    ...
    Section~\ref{sec:results-box} and Chapter~6 address. Rendered by
...
the promoted configuration ($n=40$) and tracks the two Chapter~6 fixes ---
...
guard of Section~7.2 (Finding~\#37).
...
rescale). Chapter~6 investigated this under pre-registration (Finding~\#45, the
...
not to matter; this section checks it (Finding~\#44). On the shipped seq-08 output
...
\begin{table}[H]
  \centering
  \caption{Completion plausibility by track motion (shipped seq-08 output,
...
\SI{23}{\percent} of shipped tracks --- those with fewer than five gate-passed
