...
\begin{table}[H]
  ...
  \caption[Positioning against representative SemanticKITTI methods]{Positioning against representative SemanticKITTI methods. \textbf{The
    ...
    cited papers; this work's row is the frozen sequence~08 headline (Chapter~\ref{ch:evaluation}). RangeNet++ appears in two rows: as a standalone semantic backbone
...
This chapter has positioned the thesis between supervised deep segmentation --- accurate
but paid for in dense annotation and opacity --- and object completion, whose
reference-based metrics break down on real LiDAR, and has fixed the notation and standard
operators the later chapters build on. Chapter~\ref{ch:pipeline} turns those operators
into the detection pipeline and the completion method, and develops the donor-frame
coverage metric sketched here in full.
