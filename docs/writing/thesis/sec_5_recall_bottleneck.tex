...
% and their tables use typed numerals ("Chapter~4", "Section~4.3", "Table~4.1")
...
\section{The Recall Bottleneck}
\label{ch:recall-bottleneck}

Chapter~4 established two facts about the headline operating point: the pipeline is
...
report a search of repair strategies that all fail on held-out data
...
carried to Chapter~8. Unless stated otherwise, the recall denominator throughout is
the one defined in Section~4.1 --- ground-truth car instances with at least
\num{10} points surviving preprocessing.

\subsection{The shortfall is a clustering failure, not a classifier failure}
\label{sec:recall-diagnosis}

The recall ceiling is upstream of the learned classifier. The stage decomposition
of Section~4.3 (Findings~\#47/\#49) makes this direct: geometric clustering with the
...
\subsection{Root cause: HDBSCAN splits large, close cars}
...
reported in Section~4.4 (Finding~\#50): the nearest bin has \emph{lower} recall than
...
\subsection{Repair strategies fail to generalise}
\label{sec:repairs}
...
\emph{reassemble} fragments after HDBSCAN has split them. We tried both;
Table~\ref{tab:strategies} collects the ones with a recorded quantitative outcome.
All were evaluated against the same pre-promotion sequence-00 baseline (full
pipeline, classifier and track filter on; $F_1=\num{0.844}$), and the load-bearing
ones were re-checked on the held-out sequence~08.
...
\begin{table}[H]
  ...
  \caption{Recall-repair strategies against the pre-promotion baseline (full
    ...
    finding numbers. A sixth strategy, temporal point accumulation, is discussed in
    the text but omitted here: it is single-sourced to a reverted feature in the
    session log, not reproducible under the research freeze, and belongs in prose
    rather than the main results table.}
...
\subsection{What worked: clustering resolution, and the corrected ceiling}
...
Section~4.5). A coarser grid closes the small intra-car density gaps that HDBSCAN was
...
merge problem; and the clustering-method benchmark of Section~4.3 (Finding~\#43)
...
learned detector, which is outside the scope of this thesis (Chapter~8). The
...
(Section~\ref{sec:split-analysis}). A search of repair strategies --- higher minimum
cluster size, fragment merging, adaptive and BEV clustering, temporal aggregation ---
all fail on held-out data, for a shared reason: density-based clustering has no notion
of objectness, so any criterion loose enough to keep a split car whole also fuses
adjacent cars, and reassembling fragments after the split faces the same ambiguity
(Section~\ref{sec:repairs}). The one effective lever was clustering
...
to the limitations and future-work discussion of Chapter~8.
