...
section of Chapter~\ref{ch:evaluation} rather than claiming real-time capability. The one
...
and Chapter~\ref{ch:evaluation} reports a controlled benchmark (Finding~\#43) confirming it beats
...
(\SI{6}{\percent}). Its individual gates are ablated in Chapter~\ref{ch:evaluation}; here we note
...
\begin{figure}[htbp]
  ...
  \caption[The training-input distribution fix]{The training-input distribution fix (Finding~\#26): one ShapeNet car in
...
\begin{figure}[htbp]
  ...
  \caption[The inference-normalisation fix]{The inference-normalisation fix (Finding~\#26) in bird's-eye view at
...
width (Section~\ref{sec:donor-validation}, Finding~\#44), and the clouds are filled rather than CAD-clean;
