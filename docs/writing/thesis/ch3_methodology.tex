...
\begin{figure}[H]
  ...
  \caption[Detection pipeline]{The detection pipeline, grouped into two phases (dashed): a
    ...
    (\num{4068} frames, AMD Ryzen 7 7800X3D + RTX 3070~Ti), and are
...
clusters per frame on sequence~08, varying with scene density.
...
comparison showed the synthetic pretraining to be redundant
...
\begin{figure}[htbp]
  ...
  \caption[Completions on real seq-08 cars]{What the final completion path produces on real seq-08 cars, four
    ...
    width inflation discussed in Section~\ref{sec:donor-validation}).}
...
densifies the visible surface but adds almost no genuinely new surface: the
...
surface and predicts nothing new --- therefore scores the lowest (best) CD
...
mirrored / completed clouds in every case. Its consequence is that when the geometry work
...
For a static car observed across many frames, we complete it from one
frame's pipeline cluster and then score the completed cloud only against surface
that the other frames --- the \emph{donor} frames --- saw but the input
frame did not (Figure~\ref{fig:donor-principle}). Two properties make this a fair
test of completion:

\begin{itemize}
  \item The raw partial covers none of that donor-only surface by
  ...
  \item The reference is built from ground-truth labelled points only, never
    ...
    the thing we measure against. The metric is a paired comparison of
...
to the accumulation in full, mixing the near surface the input already observed with
...
frame except the input frame, transformed to the world frame and
voxelised at \SI{3}{\centi\meter}.

\paragraph{Novel set (the visibility mask).} From the donor cloud we keep only
points that lie at least $\tau$ from every input point (nearest-neighbour
