...
Chapter~\ref{ch:completion-method} for completion).
...
against surface hallucinated outside the car. Chapter~\ref{ch:completion-eval} adopts the
...
also, as Chapter~\ref{ch:recall-bottleneck} shows, what splits a single car when an internal
...
Chapter~\ref{ch:completion-eval} shows it is invalid as a real-data \emph{score}. Detection is
...
(Chapter~\ref{ch:detection-eval}). Completion is scored by the \emph{donor coverage} developed
in Chapter~\ref{ch:completion-eval}. Given the input cluster $I$, a \emph{donor cloud} $D$ of
