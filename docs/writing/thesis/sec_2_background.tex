...
% ("Chapter~3", "Chapter~5", "Chapter~7") because the chapter files do not share a label
...
% explained in Chapter~3 (pipeline) and Chapter~6 (completion) and are NOT re-derived
...
%     seq 08; Chapter~4, Finding~#34) with the point-IoU>=0.3 protocol named
...
the method chapters (Chapter~3 for detection, Chapter~6 for completion); here the aim
...
which its car/not-car classifier is built (Chapter~3). The classifier, however, is a
...
(Chapter~5). The positioning below should be read with that trade in view: the
...
(Chapter~6), and its comparison of PCN against a transformer alternative found no
decisive real-data advantage for the heavier model (Chapter~7). Second, and more
...
not provide: the accumulation is itself one-sided. Chapter~7 shows this makes pseudo
...
\begin{table}[H]
  ...
  \caption{Positioning against representative SemanticKITTI methods. \textbf{The
    ...
    cited papers; this work's row is the frozen sequence~08 headline (Chapter~4,
...
$\ge$-10-surviving-points denominator of Chapter~4) is well below the accuracy of those
...
training stage for detection (its runtime is reported in Section~4.5), and every stage of
its decision is inspectable, so its
failure modes can be --- and are --- root-caused rather than left opaque (the recall
shortfall is traced to clustering fragmentation, not classifier error, in Chapter~5).
...
--- it must, because those clusters are the input to the completion stage (Chapter~6) ---
...
to those choices --- is examined directly in Section~4.1.
...
invalid (Chapter~7), the root-cause analysis of the recall bottleneck together with the
chain of negative repair results (Chapter~5), and the finding that synthetic
pretraining is redundant for the classifier at scale (Chapter~4). The positioning table
