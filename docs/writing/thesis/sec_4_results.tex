...
% sibling sections use typed numerals ("Section~4.1", "Chapter~5") exactly as
...
\label{ch:detection-eval-results}

\subsection{Detection Results}
\label{sec:results-detection}

The headline operating point is the sequence~08 row of Table~4.1
(Section~4.1): precision \num{0.905}, recall \num{0.730}, $F_1$ \num{0.808}, and
...
root-cause the splitting mechanism in Chapter~5; here we establish only that
...
\begin{figure}[htbp]
  ...
  \caption{Per-frame detection behaviour over sequence~08. False positives (lower
    ...
    ground-truth cars. The structural recall limit is root-caused in Chapter~5
...
protocol of Section~4.1 unless a deviation is stated explicitly.

\subsubsection{Stage decomposition: what the classifier and track filter buy}
...
recall it forgoes here is small next to the recall it never had (Chapter~5). The
...
\subsubsection{Synthetic pretraining is redundant (Stage~A ablation)}
...
\subsubsection{Clustering method: HDBSCAN earns its cost}
...
cluster or to clear the \num{10}-point eligibility rule (Section~4.1).
...
the classical CPU stages account for about \SI{94}{\percent}, leaving the learned
components (classifier and tracker) at only $\sim$\SI{5.5}{\percent}. The
