...
which sets the agenda for the future work of Chapter~\ref{ch:conclusion}.
...
donor-frame occluded-side coverage metric (Chapter~\ref{ch:completion-eval}). We are not aware of a prior metric
...
root-caused to HDBSCAN splitting large, close cars into fragments (Chapter~\ref{ch:recall-bottleneck}), and that
...
holding on sequence~00 as well (Chapter~\ref{ch:recall-bottleneck}). The
...
The recall ceiling is architectural, not a tuning failure (Chapter~\ref{ch:recall-bottleneck}). Coarsening the
...
is a different system and is left to future work (Chapter~\ref{ch:conclusion}). The value of Chapter~\ref{ch:recall-bottleneck} is to
...
ends, and Chapter~\ref{ch:conclusion} turns the most consequential of those boundaries --- the clustering
