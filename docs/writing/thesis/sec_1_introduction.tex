...
Chapters~\ref{ch:detection-eval} and~\ref{ch:completion-eval} take up.
...
\begin{description}
  \item[RQ1.] How can the quality of point-cloud completion be evaluated on
    ...
    is prior to any claim that completion helps. It is answered in Chapter~\ref{ch:completion-eval}.
  \item[RQ2.] Does learned completion recover genuinely \emph{unseen} surface --- as
    ...
    answered in Chapter~\ref{ch:completion-eval}, with the held-out outcome reported exactly as it fell.
  \item[RQ3.] What limits the recall of a training-free clustering-plus-classifier
    ...
    The first half is answered in Chapter~\ref{ch:recall-bottleneck}, the second in Chapter~\ref{ch:detection-eval}.
\end{description}
...
\begin{enumerate}
  \item \textbf{A leakage-free metric for completion quality on real LiDAR.} The
    ...
    developed in Chapter~\ref{ch:completion-eval}.
  \item \textbf{A root-cause account of the recall bottleneck, with a chain of
    ...
    own right in Chapter~\ref{ch:recall-bottleneck}.
  \item \textbf{Evidence that synthetic pretraining is redundant for the classifier
    ...
    Section~\ref{sec:intro-scope}, and is presented in Chapter~\ref{ch:detection-eval}.
\end{enumerate}
...
The remainder of the thesis is organised as follows. Chapter~\ref{ch:background} (Background and
...
comparison. Chapter~\ref{ch:pipeline} (The Detection Pipeline) describes every stage and the
classifier's training. Chapter~\ref{ch:detection-eval} (Detection Evaluation) sets out the evaluation
protocol and reports the seq-08 results, the ablations, the distance breakdown, and
the runtime. Chapter~\ref{ch:recall-bottleneck} (The Recall Bottleneck) root-causes the recall shortfall to
clustering fragmentation and reports the chain of negative repair strategies.
Chapter~\ref{ch:completion-method} (Point-Cloud Completion) covers the completion method and the debugging
story behind it. Chapter~\ref{ch:completion-eval} (Completion Evaluation) introduces the donor-frame
...
pre-registered held-out replication. Chapter~\ref{ch:discussion} (Discussion and Limitations) names
every weakness the work is subject to, and Chapter~\ref{ch:conclusion} (Conclusion and Future Work)
