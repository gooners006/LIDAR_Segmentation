...
%     operating point (leakage-free fold-08 confirms the shipped seq-08 numbers)
...
\begin{itemize}
  \item \textbf{Sequence 08 (reported operating point; \num{4071} frames).} The
    ...
    of the \emph{shipped} classifier. Sequence~08 was never used to tune the
    classical stages. The shipped classifier was historically selected on
...
sequence~08 under this protocol, produced by the shipped classifier (the one the
...
\begin{table}[H]
  ...
  \caption{Reported detection operating point under the frozen protocol, using
    the shipped classifier. Sequence~08 is the largest labelled sequence and was
...
the shipped classifier was historically selected on. To show that this selection
...
\begin{table}[H]
  ...
  \caption{Leave-one-sequence-out cross-validation over all eleven labelled
    sequences (full sequences, $\tau=0.3$, shipped configuration). Each fold's
...
two checkpoint-selection rules}: the shipped model and the fold-08 model share the
same architecture, the same ten training sequences (\numrange{00}{07},
\numrange{09}{10}), and the same from-scratch \num{15}-epoch regime, and differ
only in which epoch is frozen --- the shipped model takes the epoch that scored
...
trains on \SI{95}{\percent} of the shipped model's clusters (a \num{5}-percent
